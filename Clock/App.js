...
Ext.define('CustomApp', {
    ...
    _updateSeconds: function( count ) {
        ...
        Ext.Array.each( this.seconds_array, function( second, index ) {
            if ( index > count ) {
                second.animate( { to: { fill: '#fff' } } );
            } else {
                second.animate( { to: { fill: me.past_color } } );
            }
        });
    },
    _updateMinutes: function( count ) {
        ...
        Ext.Array.each( this.minutes_array, function( minute, index ) {
            if ( index >= 6 ) {
                ...
                if ( sub_index > units ) {
                    minute.animate( { to: { fill: '#fff' } } );
                } else if ( sub_index === units ) { 
                    minute.animate( { to: { fill: me.current_color } } );
                    me.minute_units_text.animate( { 
                        to: { 
                    ...
                    minute.animate( { to: { fill: me.past_color } } );
                }
            } else {
                // tens
                if ( index > tens ) {
    ...
    _updateHours: function( count ) {
        ...
        Ext.Array.each( this.hours_array, function( hour, index ) {
            if ( index > count ) {
                hour.animate( { to: { fill: '#fff' } } );
            } else if ( index === count ) {
                hour.animate( { to: { fill: me.current_color } } );
                
                me.hour_text.animate( { 
                    to: { 
                ...
                hour.animate( { to: { fill: me.past_color} } );
            }
        });
    }
});
